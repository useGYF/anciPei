    ...
    <div class="main">
        ...
        <div class="table_video">
            <el-table :data="tableData" border @current-change="RowCurrentChange" style="width: 100%">
                ...
                <el-table-column prop="casesNum" label="案件数量"></el-table-column>
            </el-table>
        </div>
...
<script>
    ...
    export default {
        ...
        mounted() {
            //页面初始化数据
            api.GetVideoDeviceInfoR('','','2').then(res =>{
                //console.log(res);
        ...
        methods: {
            ...
            RowCurrentChange(val) {
                this.currentRow = val;
                // 地图联的
                bus.$emit('locationClick', 'layer_sp_slw', this.currentRow ,this.type,true);
            },
            //设置数据
            SetDataList(data) {
                this.tableData = [];
                ...
                data.forEach(item => {
                    ...
                    tableData.videoName = item.name;//
                    tableData.casesNum = item.sunnum ? item.sunnum : '0';//
            ...
            searchData() {
                ...
                api.GetVideoDeviceInfoR('' ,gsname,'2').then(res =>{
                    //console.log(res);
                    let shoulist= res.data.data;
                    this.InitializationDataMethod(shoulist);
                })
