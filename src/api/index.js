...
import {
  ...
  removeInspectorCasePageResource,//督查案件删除
} from './resource'
...
//  	 let params = { "":""};
  ...
  //添加案件
  addCaseList(id,companyId,name, sendPeople, feedbackPeople, caseType, sendTime, feedbackTime, videoFootage, caseDescript, feedbackDescript,feedbackFootage,source) {
    let params = {
        'id':id,
  ...
  //督查案件删除
  removeInspectorCasePage(id){
    let params = {
      'id':id,
    };
    let FormatParams = Qs.stringify(params);//转换数据格式
    return axios.post(removeInspectorCasePageResource, FormatParams, {
        headers: {'Content-Type': 'application/x-www-form-urlencoded'}
      }
    )
  },
