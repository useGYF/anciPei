      ...
      * */
      ...
      loadXCYTrack(data) {
        ...
        for (let i = 0, length = data.length; i < length; i++) {
          // var test=this.transformFun([data[i].x,data[i].y]);
          pointsarray.push(new BMap.Point(data[i].x, data[i].y));
        }
        var polyline = new BMap.Polyline(pointsarray, {strokeColor: "blue", strokeWeight: 2, strokeOpacity: 0.5});   //创建折线
        this.currentTrackLine = polyline;
        this.map.addOverlay(polyline);   //增加折线
        this.setZoom(pointsarray);
      },

      //清除所有轨迹
      clearTrackLine() {
        let polyline = this.currentTrackLine
        if (polyline)
...
//        }
      ...
      setMouseLabel(tg) {
        ...
        let lt = tg.layerType;
        if (['LAYER_YQD', 'LAYER_YJ', 'LAYER_QY', 'LAYER_SP_SLW', 'LAYER_SP_CY', 'LAYER_SP_HD', 'LAYER_SP_VOC', 'LAYER_SP_GD', 'LAYER_SP_GKW'].indexOf(lt.toUpperCase()) > -1) {
          ...
          switch (ptType) {
            ...
            case 'LAYER_GS':
              ...
              pms = {
                ...
                'isLf': attributes.isLf,
                'dataLevel': (attributes.dataLevel || null)
              };
              displayName = 'pointname';
              break;
            case 'LAYER_GD':
      ...
      setCyInfoWindow(data) {
        ...
        return `  <div class="form">
            ...
            <label><span>监测浓度</span><div>${data.concentrationOut || '暂无'}</div></label>
            <label><span>更新时间</span><div>${data.monitorTime || '暂无'}</div></label>
        </div>`;
      },
      //散乱污监控
